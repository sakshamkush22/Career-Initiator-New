...
@WebServlet(name = "AddCourseDetailsSrv", urlPatterns = {"/AddCourseDetailsSrv"})
public class AddCourseDetailsSrv extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        bean.setCOURSE_ID(Integer.parseInt(request.getParameter("cid")));
        ...
        int r=0;
        try{
         r= dao.addCourseDetails(bean);
        }catch(Exception e){e.printStackTrace();}
        
        if(r>0){
          response.sendRedirect(request.getContextPath()+"/AdminPages/view-courseDetails.jsp");
        }else{
          response.sendRedirect(request.getContextPath()+"/AdminPages/add-courseDetails.jsp");
        }
