...
@WebServlet(name = "UpdateExamSrv", urlPatterns = {"/UpdateExamSrv"})
public class UpdateExamSrv extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        bean.setEXAM_ID(Integer.parseInt(request.getParameter("exmid")));


        
        int r=0;
        try{
         r= dao.updateExam(bean);
        }catch(Exception e){e.printStackTrace();}
        
        if(r>0){
          response.sendRedirect(request.getContextPath()+"/AdminPages/view-exam.jsp");
        }else{
          response.sendRedirect(request.getContextPath()+"/AdminPages/update-exam.jsp");
        }
