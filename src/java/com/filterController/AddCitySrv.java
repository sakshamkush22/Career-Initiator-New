...
@WebServlet(name = "AddCitySrv", urlPatterns = {"/AddCitySrv"})
public class AddCitySrv extends HttpServlet {


    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        
        CollegeByCity bean = new CollegeByCity();
        CollegeByCityImpl dao = new CollegeByCityImpl();
        
        bean.setCITY_NAME(request.getParameter("name"));
        ...
        int r=0;
        try{
         r= dao.addCollegeByCity(bean);
        }catch(Exception e){e.printStackTrace();}
        
        if(r>0){
          response.sendRedirect(request.getContextPath()+"/AdminPages/view-city.jsp");
        }else{
          response.sendRedirect(request.getContextPath()+"/AdminPages/add-city.jsp");
        }
