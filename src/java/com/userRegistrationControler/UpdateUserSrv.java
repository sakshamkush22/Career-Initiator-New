...
@WebServlet(name = "UpdateUserSrv", urlPatterns = {"/UpdateUserSrv"})
public class UpdateUserSrv extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        bean.setWhatssapNo(request.getParameter("wstnum"));
        ...
        int r=0;
        try{
         r= dao.updateUser(bean);
        }catch(Exception e){e.printStackTrace();}
        
        if(r>0){
           response.sendRedirect("user-profile.jsp?msg=Profile Updated Successfully");
        }else{
         response.sendRedirect("user-profile-update.jsp?msg=Some Error Accour");
        }
